      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-10">
        <div className="lg:col-span-4">
          <div className="bg-slate-900/50 backdrop-blur-xl p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border border-slate-800/50 relative overflow-hidden">
            ...
            <h2 className="text-lg md:text-xl font-bold mb-6 md:mb-8 text-white relative z-10">Set New Target</h2>
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 md:gap-5 relative z-10">
              <input type="text" required value={name} onChange={(e) => setName(e.target.value)} className="w-full p-3.5 md:p-4 bg-slate-950/50 border border-slate-800/80 rounded-2xl outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50 text-white placeholder-slate-600 transition-all text-sm md:text-base" placeholder="Goal Name (e.g., Japan Trip)" />
              <div className="relative group">
                 <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-500 font-bold text-sm md:text-base">RM</div>
                 <input type="number" step="0.01" required value={targetAmount} onChange={(e) => setTargetAmount(e.target.value)} className="w-full pl-12 p-3.5 md:p-4 bg-slate-950/50 border border-slate-800/80 rounded-2xl outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50 text-white placeholder-slate-600 transition-all text-sm md:text-base" placeholder="Target Amount" />
              </div>
              <input type="date" required value={targetDate} onChange={(e) => setTargetDate(e.target.value)} className="w-full p-3.5 md:p-4 bg-slate-950/50 border border-slate-800/80 rounded-2xl outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50 text-white placeholder-slate-600 transition-all [color-scheme:dark] text-sm md:text-base" />
          ...
          <div className="bg-slate-900/30 backdrop-blur-xl p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border border-slate-800/30 h-full">
            ...
            <div className="flex flex-col gap-4 md:gap-6">
              {goals.map((goal) => {
                ...
                return (
                  <div key={goal.id} className="bg-slate-900/50 p-5 md:p-6 rounded-[1.5rem] md:rounded-3xl border border-slate-800/50 hover:bg-slate-800/40 transition-all relative overflow-hidden group">
                    <div className="absolute top-0 right-0 w-32 h-full bg-blue-500/5 blur-2xl"></div>
                    <div className="flex justify-between items-start mb-4 relative z-10">
                      <div>
                        <h3 className="text-lg md:text-xl font-bold text-white tracking-wide pr-2">{goal.name}</h3>
                      </div>
                    ...
                    <div className="w-full bg-slate-950/80 rounded-full h-3 md:h-4 mb-5 border border-slate-800/80 relative z-10">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 text-sm font-bold text-slate-400 relative z-10">
                      <div className="flex items-center gap-2 w-full sm:w-auto">
                         ...
                         <button onClick={() => handleAddFunds(goal.id, goal.name, goal.current_amount)} className="bg-blue-600/20 text-blue-400 hover:bg-blue-600 hover:text-white px-3 md:px-4 py-2.5 md:py-2 rounded-lg transition-colors text-xs md:text-sm font-bold whitespace-nowrap">Add</button>
                      </div>
                      <div className="text-left sm:text-right w-full sm:w-auto flex flex-row justify-between sm:block text-xs md:text-sm">
                        ...
                        <span className="text-slate-500">RM {Number(goal.target_amount).toLocaleString(undefined, {maximumFractionDigits:0})}</span>
                      </div>
                    </div>
                  </div>
                );
