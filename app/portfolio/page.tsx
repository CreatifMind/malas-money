...
export default function Portfolio() {
  ...
  const updateLivePrices = async (currentData: any[]) => {
    ...
    const usdToMyrRate = 4.7; // Approx conversion for US Stocks
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 2. The Magic: Auto-Deduct from Liquid Cash via Transactions!
    if (!error) {
  ...
  return (
    <div className="min-h-full bg-[#0B0F19] rounded-[2.5rem] p-8 m-4 shadow-2xl text-slate-50 animate-in fade-in duration-500">
      <header className="mb-10 flex justify-between items-end px-4">
        <div>
          <h1 className="text-4xl font-extrabold tracking-tight text-white">Portfolio</h1>
          <p className="text-slate-400 mt-1 font-medium">Your investments, real-time.</p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        <div className="relative overflow-hidden bg-slate-900/50 backdrop-blur-xl p-8 rounded-[2rem] border border-slate-800/50 shadow-xl group">
          ...
          <h3 className="text-sm font-bold text-slate-500 mb-2 uppercase tracking-wider relative z-10">Live Value</h3>
          <p className="text-4xl font-extrabold text-white relative z-10">RM {summary.currentValue.toLocaleString(undefined, {minimumFractionDigits:2, maximumFractionDigits:2})}</p>
        </div>
        <div className="bg-slate-900/40 backdrop-blur-xl p-8 rounded-[2rem] border border-slate-800/50">
          <h3 className="text-sm font-bold text-slate-500 mb-2 uppercase tracking-wider">Capital Invested</h3>
          <p className="text-3xl font-bold text-slate-300">RM {summary.totalInvested.toLocaleString(undefined, {minimumFractionDigits:2, maximumFractionDigits:2})}</p>
        </div>
        <div className="bg-slate-900/40 backdrop-blur-xl p-8 rounded-[2rem] border border-slate-800/50">
          <h3 className="text-sm font-bold text-slate-500 mb-2 uppercase tracking-wider">All-Time Return</h3>
          <p className={`text-3xl font-extrabold ${summary.totalProfit >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-4">
          <div className="bg-slate-900/50 backdrop-blur-xl p-8 rounded-[2.5rem] border border-slate-800/50">
            <h2 className="text-xl font-bold mb-8 text-white">Execute Trade</h2>
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              
              <div className="relative group">
                <select value={assetClass} onChange={(e) => setAssetClass(e.target.value)} className="w-full p-4 bg-slate-950/50 border border-slate-800/80 rounded-2xl outline-none focus:border-sky-500/50 focus:ring-1 focus:ring-sky-500/50 text-white appearance-none cursor-pointer">
              ...
              <input type="text" required value={assetName} onChange={(e) => setAssetName(e.target.value)} className="w-full p-4 bg-slate-950/50 border border-slate-800/80 rounded-2xl outline-none focus:border-sky-500/50 focus:ring-1 focus:ring-sky-500/50 text-white placeholder-slate-600" placeholder="Asset Name (e.g., Apple)" />

              {assetClass === 'Stock' && (
                <input type="text" required value={ticker} onChange={(e) => setTicker(e.target.value)} className="w-full p-4 bg-slate-950/50 border border-slate-800/80 rounded-2xl outline-none focus:border-sky-500/50 focus:ring-1 focus:ring-sky-500/50 text-white placeholder-slate-600" placeholder="Ticker (e.g., AAPL)" />
              )}

              <div className="p-5 bg-sky-500/5 rounded-2xl border border-sky-500/20 mt-2 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-sky-500/10 rounded-full blur-2xl"></div>
                <label className="block text-sm font-bold text-sky-400 mb-2 relative z-10">Total Capital (RM)</label>
                <input type="number" step="0.01" required value={totalInvestedInput} onChange={(e) => setTotalInvestedInput(e.target.value)} className="w-full p-3 text-2xl font-bold text-white bg-transparent border-b border-sky-500/30 outline-none focus:border-sky-400 placeholder-slate-700 relative z-10" placeholder="0.00" />
                <p className="text-xs text-slate-400 mt-3 relative z-10">App will auto-calculate quantity based on live market price.</p>
              </div>

              <button type="submit" className="mt-4 w-full bg-sky-500 hover:bg-sky-400 text-slate-950 font-extrabold py-4 rounded-2xl transition-all shadow-[0_0_20px_rgba(14,165,233,0.15)] active:scale-[0.98]">
                {status || "Buy Asset"}
              </button>
            </form>
          </div>
        ...
        <div className="lg:col-span-8">
          <div className="bg-slate-900/30 backdrop-blur-xl p-8 rounded-[2.5rem] border border-slate-800/30 h-full">
            <div className="flex justify-between items-center mb-8">
              ...
              <button onClick={() => updateLivePrices(investments)} disabled={isRefreshing} className="flex items-center gap-2 text-sm font-bold text-sky-400 bg-sky-500/10 hover:bg-sky-500/20 px-4 py-2.5 rounded-xl transition-all disabled:opacity-50">
                <svg className={`w-4 h-4 ${isRefreshing ? 'animate-spin' : ''}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 12a9 9 0 1 1-9-9c2.52 0 4.93 1 6.74 2.74L21 8"></path><polyline points="21 3 21 8 16 8"></polyline></svg>
              ...
              <table className="w-full text-left border-collapse whitespace-nowrap">
                <thead>
                  <tr className="border-b border-slate-800/80 text-xs text-slate-500 uppercase tracking-wider">
                    ...
                    <th className="pb-4 font-bold">Invested</th>
                    <th className="pb-4 font-bold">Live Price</th>
                    <th className="pb-4 font-bold">Value</th>
                    <th className="pb-4 font-bold">Return</th>
                    <th className="pb-4 font-bold text-slate-600">Qty</th>
                    ...
                    return (
                      <tr key={inv.id} className="hover:bg-slate-800/40 transition-colors group">
                        <td className="py-4 pr-4">
                          <p className="font-extrabold text-white">{inv.ticker_symbol || inv.asset_name}</p>
                          <div className="flex items-center gap-2 mt-1">
                            <span className={`text-[9px] uppercase tracking-wider font-bold px-2 py-0.5 rounded-full border ${getBadgeStyle(inv.asset_class)}`}>{inv.asset_class}</span>
                          </div>
                        </td>
                        <td className="py-4 text-slate-300 font-medium">RM {totalCost.toLocaleString(undefined, {minimumFractionDigits:2})}</td>
                        <td className="py-4 font-bold text-sky-400">RM {Number(inv.current_price).toLocaleString(undefined, {minimumFractionDigits:2})}</td>
                        <td className="py-4 font-bold text-white">RM {totalValue.toLocaleString(undefined, {minimumFractionDigits:2})}</td>
                        <td className="py-4">
                           <p className={`text-sm font-extrabold ${profit >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                            {profit >= 0 ? '+' : ''}{profitPercent.toFixed(2)}%
                          </p>
                        </td>
                        <td className="py-4 text-xs text-slate-600 font-mono">{Number(inv.quantity).toLocaleString(undefined, {maximumFractionDigits: 4})}</td>
                        <td className="py-4 text-right">
                          <button onClick={() => handleDelete(inv.id)} className="opacity-0 group-hover:opacity-100 p-2 text-slate-500 hover:text-rose-400 hover:bg-rose-500/10 rounded-xl transition-all">
